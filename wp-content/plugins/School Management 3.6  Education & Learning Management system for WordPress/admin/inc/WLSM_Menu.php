...
class WLSM_Menu {
		...
		if ( WLSM_Helper::lm_valid() ) {
			...
			if ( $current_school = $user_info['current_school'] ) {
				...
				if ( in_array( $role, array_keys( WLSM_M_Role::get_roles() ) ) ) {
					...
					// Academic - Group.
					if ( WLSM_M_Role::check_permission( array( 'manage_classes', 'manage_subjects', 'manage_timetable', 'manage_attendance', 'manage_study_materials', 'manage_notices' ), $permissions ) ) {
					...
					if ( WLSM_M_Role::check_permission( array( 'manage_admins', 'manage_roles', 'manage_employees' ), $permissions ) ) {
						...
						if ( WLSM_M_Role::check_permission( array( 'manage_employees' ), $permissions ) ) {
							// School - Employees.
							$school_staff_employees_submenu = add_submenu_page( WLSM_MENU_STAFF_ADMINISTRATOR, esc_html__( 'Staff', 'school-management' ), esc_html__( 'Staff', 'school-management' ), 'read', WLSM_MENU_STAFF_EMPLOYEES, array( 'WLSM_Menu', 'school_staff_employees' ) );
	...
	public static function menu_page_assets() {
		...
		wp_enqueue_style( 'wlsm-print-preview', WLSM_PLUGIN_URL . 'assets/css/print/wlsm-preview.css', array(), '3.6', 'all' );
		wp_enqueue_style( 'wlsm-admin', WLSM_PLUGIN_URL . 'assets/css/wlsm-admin.css', array(), '3.6', 'all' );
		wp_enqueue_style( 'wlsm-school-header', WLSM_PLUGIN_URL . 'assets/css/wlsm-school-header.css', array(), '3.6', 'all' );
		...
		wp_enqueue_script( 'wlsm-admin', WLSM_PLUGIN_URL . 'assets/js/wlsm-admin.js', array( 'jquery', 'jquery-form' ), '3.6', true );
