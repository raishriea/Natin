...
class WLSM_M_School {
	...
	public static function get_school( $id ) {
		global $wpdb;
		$school = $wpdb->get_row( $wpdb->prepare( 'SELECT s.ID, s.enrollment_prefix, s.enrollment_base, s.enrollment_padding FROM ' . WLSM_SCHOOLS . ' as s WHERE s.ID = %d', $id ) );
		return $school;
	}
	...
	public static function fetch_school( $id ) {
		global $wpdb;
		$school = $wpdb->get_row( $wpdb->prepare( 'SELECT s.ID, s.label, s.phone, s.email, s.address, s.is_active, s.enrollment_prefix, s.enrollment_base, s.enrollment_padding FROM ' . WLSM_SCHOOLS . ' as s WHERE s.ID = %d', $id ) );
		return $school;
	}
