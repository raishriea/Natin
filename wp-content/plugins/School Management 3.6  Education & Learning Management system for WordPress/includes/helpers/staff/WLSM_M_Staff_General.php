...
class WLSM_M_Staff_General {
	...
	public static function get_student( $school_id, $session_id, $id, $skip_transferred = false, $only_active = false ) {
		global $wpdb;
		...
		if ( ! $skip_transferred ) {
			$student = $wpdb->get_row( $wpdb->prepare( 'SELECT sr.ID, sr.photo_id, sr.user_id, sr.parent_user_id, se.class_school_id, cs.class_id FROM ' . WLSM_STUDENT_RECORDS . ' as sr JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id WHERE cs.school_id = %d AND sr.session_id = %d AND sr.ID = %d' . $where, $school_id, $session_id, $id ) );
		} else {
			$student = $wpdb->get_row( $wpdb->prepare( 'SELECT sr.ID, sr.photo_id, sr.user_id, sr.parent_user_id, se.class_school_id, cs.class_id FROM ' . WLSM_STUDENT_RECORDS . ' as sr JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id LEFT OUTER JOIN ' . WLSM_TRANSFERS . ' as tf ON tf.from_student_record = sr.ID WHERE cs.school_id = %d AND sr.session_id = %d AND sr.ID = %d' . $where . ' AND tf.ID IS NULL', $school_id, $session_id, $id ) );
		}
	...
	public static function get_certificate_students( $school_id, $session_id, $certificate_id ) {
		...
		$students = $wpdb->get_results(
			$wpdb->prepare( 'SELECT cfsr.ID, cfsr.certificate_number, sr.ID as student_id, sr.enrollment_number, sr.roll_number, sr.name as student_name, sr.phone, c.label as class_label, se.label as section_label FROM ' . WLSM_CERTIFICATE_STUDENT . ' as cfsr 
