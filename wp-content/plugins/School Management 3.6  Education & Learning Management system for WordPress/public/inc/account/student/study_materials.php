...
$study_materials_per_page = 10;

$study_materials_query = 'SELECT sm.ID, sm.label as title, sm.description, sm.attachments, sm.created_at FROM ' . WLSM_CLASS_SCHOOL_STUDY_MATERIAL . ' as cssm JOIN ' . WLSM_STUDY_MATERIALS . ' as sm ON sm.ID = cssm.study_material_id WHERE cssm.class_school_id = %d GROUP BY sm.ID';
